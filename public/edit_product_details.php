...
if ($_POST) {
    ...
    if(isset($_POST["submit_post"])) {
        ...
        $user_detail = $login->get_user_details();
        $current_user_id = $user_detail['auth_id'];
        $currnet_user_item ;
        ...
        if (isset($_GET['id'])) {
            ...
            $currnet_user_item = $item[0];
            $has_error = true;
            $error['no_id'] = "We did not find the page you wanted.";
        }

        $test = $currnet_user_item->product_name;
        if ($query_user_id == $currnet_user_item->user_id){
        echo '<html>
        <head>
        	<meta charset="UTF-8">
            <title>ECU Consignment Edit Products Page</title>
            <meta charset="utf-8">
                    ...
                    <table border="0">
                        <thead>
                            <tr>
                                ...
                                <td><input type="text" name="product_name" value="'.$test.'" placeholder="Product Name" pattern="[a-z A-Z]+"></td>
